...
def addHead(dataDownloadPath, dataPath):
    with open(dataDownloadPath, "r+", encoding="utf-8") as f:
        data = f.read().split("\n")
    data.insert(0, "400000 100")
    with open(dataPath, "w+", encoding="utf-8") as f:
        f.write("\n".join(data))

def get_neighbor_for_vocab(word2id, dataPath, dataDownloadPath, dict_save_path):
    ...
    if not os.path.exists(dataPath):
        addHead(dataDownloadPath, dataPath)
...
def build_instances(data_dir_path, num, context_dict):
    ...
    return train_feature_list, test_feature_list, dev_feature_list
