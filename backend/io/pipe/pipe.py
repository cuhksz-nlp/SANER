...
class Pipe:
    ...
    (2) 再建立词或字的 :class:`~backend.Vocabulary` , 并将词或字转换为index; (3)将target列建立词表并将target列转为index;
    ...
    def process(self, data_bundle: DataBundle) -> DataBundle:
        ...
        :param ~backend.DataBundle data_bundle: 需要处理的DataBundle对象
        :return:
        """
    ...
    def process_from_file(self, paths) -> DataBundle:
        """
        传入文件路径，生成处理好的DataBundle对象。paths支持的路径形式可以参考 ：:meth:`backend.io.Loader.load()`
