"""
metrics 模块实现了 backend 所需的各种常用衡量指标，一般做为 :class:`~backend.Trainer` 的参数使用。
...
class AccuracyMetric(MetricBase):
    """
    准确率Metric（其它的Metric参见 :mod:`backend.core.metrics` ）
    """
...
class SpanFPreRecMetric(MetricBase):
    ...
    def __init__(self, tag_vocab, pred=None, target=None, seq_len=None, encoding_type=None, ignore_labels=None,
        ...
        :param tag_vocab: 标签的 :class:`~backend.Vocabulary` 。支持的标签为"B"(没有label)；或"B-xxx"(xxx为某种label，比如POS中的NN)，
        ...
        if not isinstance(tag_vocab, Vocabulary):
            raise TypeError("tag_vocab can only be backend.Vocabulary, not {}.".format(type(tag_vocab)))
...
def _prepare_metrics(metrics):
    ...
    :param metrics:
    :return: List[backend.MetricBase]
    """
    _metrics = []
    if metrics:
        if isinstance(metrics, list):
            for metric in metrics:
                ...
                if isinstance(metric, MetricBase):
                    ...
                    raise TypeError(
                        f"The type of metric in metrics must be `backend.MetricBase`, not `{type(metric)}`.")
        elif isinstance(metrics, MetricBase):
            _metrics = [metrics]
        else:
            raise TypeError(f"The type of metrics should be `list[backend.MetricBase]` or `backend.MetricBase`, "
                            f"got {type(metrics)}.")
    return _metrics
