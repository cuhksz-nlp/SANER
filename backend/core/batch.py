"""
batch 模块实现了 backend 所需的 :class:`~backend.core.batch.DataSetIter` 类。
...
class DataSetIter(BatchIter):
    ...
    def __init__(self, dataset, batch_size=1, sampler=None, as_numpy=False,
        ...
        :param dataset: :class:`~backend.DataSet` 对象, 数据集
        :param int batch_size: 取出的batch大小
        :param sampler: 规定使用的 :class:`~backend.Sampler` 方式. 若为 ``None`` , 使用 :class:`~backend.SequentialSampler`.
