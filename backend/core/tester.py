"""
tester模块实现了 backend 所需的Tester类，能在提供数据、模型以及metric的情况下进行性能测试。
...
这里Metric的映射规律是和 :class:`backend.Trainer` 中一致的，具体使用请参考 :mod:`trainer 模块<backend.core.trainer>` 的1.3部分。
...
class Tester(object):
    ...
    def __init__(self, data, model, metrics, batch_size=16, num_workers=0, device=None, verbose=1, use_tqdm=True,
        ...
        :param ~backend.DataSet data: 需要测试的数据集
        :param torch.nn.module model: 使用的模型
        :param ~backend.core.metrics.MetricBase,List[~backend.core.metrics.MetricBase] metrics: 测试时使用的metrics
