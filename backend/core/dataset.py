"""
:class:`~backend.core.dataset.DataSet` 是fastNLP中用于承载数据的容器。可以将DataSet看做是一个表格，
每一行是一个sample (在fastNLP中被称为 :mod:`~backend.core.instance` )，
每一列是一个feature (在fastNLP中称为 :mod:`~backend.core.field` )。
...
在fastNLP内部每一行是一个 :class:`~backend.Instance` 对象； 每一列是一个 :class:`~backend.FieldArray` 对象。
...
--------------------------------------
    ...
    .. code-block::
        ...
        from backend import SequentialSampler
        from backend import EngChar2DPadder
    ...
    其中 :class:`~backend.DataSetIter` 是用于从DataSet中按照batch_size为大小取出batch的迭代器，
    :class:`~backend.SequentialSampler` 用于指示 :class:`~backend.DataSetIter` 以怎样的
    顺序从DataSet中取出instance以组成一个batch，
    更详细的说明请参照 :class:`~backend.DataSetIter` 和 :class:`~backend.SequentialSampler` 文档。
    ...
    二个dict中。如上例中所打印结果。分为input和target的原因是由于它们在被 :class:`~backend.Trainer` 所使用时会有所差异，
    详见  :class:`~backend.Trainer`
    ...
    (也可以在 :class:`~backend.DataSetIter` 中设置输出numpy类型，参考 :class:`~backend.DataSetIter` )。
...
--------------------------------------
    ...
    character的pad方式往往是不同的。fastNLP是通过一个叫做 :class:`~backend.Padder` 的子类来完成的。
    默认情况下，所有field使用 :class:`~backend.AutoPadder`
    。可以通过使用以下方式设置Padder(如果将padder设置为None，则该field不会进行pad操作)。
    大多数情况下直接使用 :class:`~backend.AutoPadder` 就可以了。
    如果 :class:`~backend.AutoPadder` 或 :class:`~backend.EngChar2DPadder` 无法满足需求，
    也可以自己写一个 :class:`~backend.Padder` 。
...
class DataSet(object):
    """
    fastNLP的数据容器，详细的使用方法见文档  :mod:`backend.core.dataset`
    """
    ...
    def __init__(self, data=None):
        ...
        :param data: 如果为dict类型，则每个key的value应该为等长的list; 如果为list，
            每个元素应该为具有相同field的 :class:`~backend.Instance` 。
        """
        self.field_arrays = {}
        if data is not None:
    ...
    def append(self, instance):
        ...
        :param ~backend.Instance instance: 若DataSet不为空，则instance应该拥有和DataSet完全一样的field。
    ...
    def add_fieldarray(self, field_name, fieldarray):
        ...
        :param ~backend.core.FieldArray fieldarray: 需要加入DataSet的field的内容
        :return:
        """
        if not isinstance(fieldarray, FieldArray):
            raise TypeError("Only backend.FieldArray supported.")
    ...
    def add_field(self, field_name, fields, padder=AutoPadder(), is_input=False, is_target=False, ignore_type=False):
        ...
        :param None,~backend.Padder padder: 如果为None,则不进行pad，默认使用 :class:`~backend.AutoPadder` 自动判断是否需要做pad。
    ...
    def get_field(self, field_name):
        ...
        :param str field_name: field的名称
        :return: :class:`~backend.FieldArray`
        """
    ...
    def get_all_fields(self):
        """
        返回一个dict，key为field_name, value为对应的 :class:`~backend.FieldArray`
    ...
    def set_padder(self, field_name, padder):
        ...
        :param None,~backend.Padder padder: 设置为None即删除padder, 即对该field不进行pad操作。
        """
        if field_name not in self.field_arrays:
    ...
    def split(self, ratio, shuffle=True):
        ...
        :return: [ :class:`~backend.读取后的DataSet` , :class:`~backend.读取后的DataSet` ]
        """
    ...
    def load(path):
        ...
        :return: 读取后的 :class:`~backend.读取后的DataSet`。
        """
