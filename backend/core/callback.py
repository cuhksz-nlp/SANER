r"""
callback模块实现了 backend 中的许多 callback 类，用于增强 :class:`~backend.Trainer` 类。

虽然Trainer本身已经集成了一些功能，但仍然不足以囊括训练过程中可能需要到的功能，
比如负采样，learning rate decay 和 early stop等。
为了解决这个问题，fastNLP引入了callback的机制，:class:`~backend.Callback` 是一种在Trainer训练过程中特定阶段会运行的函数集合。
关于 :class:`~backend.Trainer` 的详细文档，请参见 :mod:`trainer 模块<backend.core.trainer>`

我们将 :meth:`~backend.Trainer.train` 这个函数内部分为以下的阶段，在对应阶段会触发相应的调用::
...
如下面的例子所示，我们可以使用内置的 callback 组件，或者继承 :class:`~backend.core.callback.Callback`
定义自己的 callback 组件::
...
class Callback(object):
    """
    Callback是fastNLP中被设计用于增强 :class:`~backend.Trainer` 的类。
    如果Callback被传递给了 Trainer , 则 Trainer 会在对应的阶段调用Callback的函数，
    具体调用时机可以通过 :mod:`trainer 模块<backend.core.trainer>` 查看。
...
class EarlyStopCallback(Callback):
    """
    多少个epoch没有变好就停止训练，相关类 :class:`~backend.core.callback.EarlyStopError`
    """
...
class FitlogCallback(Callback):
    ...
    def __init__(self, data=None, tester=None, log_loss_every=0, verbose=0, log_exception=False):
        ...
        :param ~backend.DataSet,Dict[~backend.DataSet] data: 传入DataSet对象，会使用多个Trainer中的metric对数据进行验证。如果需要
            传入多个DataSet请通过dict的方式传入，dict的key将作为对应dataset的name传递给fitlog。data的结果的名称以'data'开头。
        :param ~backend.Tester,Dict[~backend.Tester] tester: Tester对象，将在on_valid_end时调用。tester的结果的名称以'tester'开头
...
class TensorboardCallback(Callback):
    ...
    .. warning::
        backend 已停止对此功能的维护，请等待 backend 兼容 PyTorch1.1 的下一个版本。
        或者使用和 backend 高度配合的 fitlog（参见 :doc:`/tutorials/tutorial_11_fitlog` ）。
    ...
    def on_batch_begin(self, batch_x, batch_y, indices):
        if "model" in self.options and self.graph_added is False:
            ...
            # from backend.core.utils import _build_args
