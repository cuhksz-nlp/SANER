...
.. code-block:: python
    ...
    from backend import CrossEntropyLoss
    from backend import AccuracyMetric
    from backend.modules.decoder import MLP
...
:mod:`Loss<backend.core.losses>` 与 :mod:`Metric<backend.core.metrics>` 都使用了通过名称来匹配相应内容的策略。如上面的例子中
...
loss被设置为了 :class:`~backend.CrossEntropyLoss` , 但在初始化的时候传入了target='label'这个参数，
:class:`~backend.CrossEntropyLoss` 的初始化参数为(pred=None, target=None, padding_idx=-100)。

这里的两个参数分别为计算CrossEntropy时需要使用到的模型的预测值与真实值。
其中 `pred` 一般来自于模型forward()的返回结果，`target` 一般是来自于DataSet中被设置为target的field。
由于每个人对真实值或者model的返回值取名并不一样，所以fastNLP的 :mod:`Loss<backend.core.losses>` 提供一种类似于映射的机制来匹配对应的值，
比如这里 :class:`~backend.CrossEntropyLoss` 将尝试找到名为'label'的内容来作为真实值得到loss；
而pred=None, 则 :class:`~backend.CrossEntropyLoss` 使用'pred'作为名称匹配预测值，
正好forward的返回值也叫pred，所以这里不需要申明pred。

尽管fastNLP使用了映射机制来使得loss的计算变得比较灵活，但有些情况下loss必须在模型中进行计算，比如使用了CRF的模型。
fastNLP中提供了 :class:`~backend.LossInForward` 这个loss。
这个loss的原理是直接在forward()的返回结果中找到loss_key(默认寻找'loss')指定的那个tensor，并使用它作为loss。
如果Trainer初始化没有提供loss则默认使用 :class:`~backend.LossInForward` 。
...
:mod:`Metric<backend.core.metrics>` 使用了与上述Loss一样的策略，即使用名称进行匹配。
...
为了解决这个问题fastNLP引入了callback的机制，:class:`~backend.Callback` 是一种在Trainer训练过程中特定阶段会运行的函数集合，
所有的 :class:`~backend.Callback` 都具有on_*(比如on_train_start, on_backward_begin)等函数。
如果 Callback 实现了该函数，则Trainer运行至对应阶段，会进行调用，例如::

    from backend import Callback, EarlyStopCallback, Trainer, CrossEntropyLoss, AccuracyMetric
    from backend.models import CNNText
...
这里，我们通过继承 :class:`~backend.Callback` 类定义了自己的 callback 的，并和内置的 :class:`~backend.EarlyStopCallback`
一起传给了 :class:`~backend.Trainer` ，增强了 :class:`~backend.Trainer` 的功能

fastNLP已经自带了很多callback函数供使用，可以参考 :mod:`backend.core.callback` 。
...
class Trainer(object):
    ...
    详细的介绍参见 :mod:`backend.core.trainer`
    """
    ...
    def __init__(self, train_data, model, optimizer=None, loss=None,
        ...
        :param train_data: 训练集， :class:`~backend.DataSet` 类型。
        ...
        :param loss: 使用的 :class:`~backend.core.losses.LossBase` 对象。当为None时，默认使用 :class:`~backend.LossInForward`
        :param sampler: Batch数据生成的顺序， :class:`~backend.Sampler` 类型。如果为None，默认使用 :class:`~backend.RandomSampler`
        ...
        :param dev_data: 用于做验证的DataSet， :class:`~backend.DataSet` 类型。
        :param metrics: 验证的评估函数。可以只使用一个 :class:`Metric<backend.core.metrics.MetricBase>` ，
            也可以使用多个 :class:`Metric<backend.core.metrics.MetricBase>` ，通过列表传入。
            如验证时取得了更好的验证结果(如果有多个Metric，以列表中第一个Metric为准)，且save_path不为None，
            则保存当前模型。Metric种类详见 :mod:`metrics模块 <backend.core.metrics>` 。仅在传入dev_data时有效。
        :param str,None metric_key:  :class:`Metric<backend.core.metrics.MetricBase>` 有时会有多个指标，
            比如 :class:`~backend.core.metrics.SpanFPreRecMetric` 中包含了'f', 'pre', 'rec'。此时需
        ...
        :param list(callbacks) callbacks: 用于在train过程中起调节作用的回调函数。比如early stop，negative sampling等可以
            通过callback机制实现。 可使用的callback参见 :mod:`callback模块 <backend.core.callback>`
        ...
        if isinstance(model, nn.parallel.DistributedDataParallel):  # 如果是分布式的
            ...
            if sampler is not None and not isinstance(sampler, (Sampler, torch.utils.data.Sampler)):
                raise ValueError(f"The type of sampler should be backend.BaseSampler or pytorch's Sampler, got {type(sampler)}")
            if sampler is None:
                sampler = RandomSampler()
            elif hasattr(sampler, 'set_batch_size'):
                sampler.set_batch_size(batch_size)
                
        # train_data: <backend.core.dataset.DataSet>
        ...
        if check_code_level > -1:
            # _check_code 是 backend 帮助你检查代码是否正确的方法 。如果你在错误栈中看到这行注释，请认真检查你的field名与模型的输入
            #   名是否匹配
