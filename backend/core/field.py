...
class FieldArray:
    def __init__(self, name, content, is_target=False, is_input=False, padder=None, ignore_type=False,
        ...
        if padder is None:
            ...
            assert isinstance(padder, Padder), "padder must be of type backend.Padder."
    ...
    def set_padder(self, padder):
        ...
        :param padder: :class:`~backend.Padder` 类型，设置为None即删除padder。
        """
    ...
    def to(self, other):
        ...
        :param  other: :class:`~backend.FieldArray` 从哪个field拷贝属性
        :return: :class:`~backend.FieldArray`
        """
        assert isinstance(other, FieldArray), "Only supports backend.FieldArray type, not {}.".format(type(other))
